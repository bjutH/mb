...
public interface Yiqi {
    ...
    String SELECT_FIELDS = "num, pad_id, hanjie, debug, check, soft_num, ps";


    @Insert({"insert into ", TABLE_NAME, "(", INSERT_FIELDS, ") values (#{product_num}, #{ps_name}"})
    void addProcess(String orderNum,String ps_name);

    @Select({"select ", SELECT_FIELDS, "from", TABLE_NAME, "where product_num = #{product_num} and process = #{ps_name}"})
